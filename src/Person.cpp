...
Profile::Profile()
{
	...
	iAutoStatus = GG_STATUS_NOT_AVAIL;
	iNeedImport = true;
	iDescription = new BString();
	iDescription->SetTo( "" );
...
#define MAGIC_HAIKU "Machibito no Ashioto tooki Ochiba ka na - Buson (1784)"
void Profile::Obfuscate(char *buffer, size_t size)
{
	// We could just memfrob() it, but why not enjoy some Haiku instead?
	const char pass[] = MAGIC_HAIKU;
	char *key;
	int i, j;
...
void Profile::Load( BString* aProfile )
{
	...
	if( cfgmesg->FindInt32( "iNumber", ( int32* ) &iNumber ) != B_OK )
		iNumber = 0;
	if( cfgmesg->FindData( "iPassword", B_RAW_TYPE, (const void **)(&buffer), &size ) != B_OK ) {
		iPassword->SetTo("");
	} else {
		// Fetch the password
		if (size <= 0) {
			...
			Obfuscate(buffer, size);
			iPassword->SetTo(buffer, size);
		}
	}
...
void Profile::Save()
{
	...
	cfgmesg->AddBool("iRememberStatus", iRememberStatus);
	{
		...
		memcpy(buffer, iPassword->String(), len);
		Obfuscate(buffer, len);
		cfgmesg->AddData("iPassword", B_RAW_TYPE, buffer, len);
		delete buffer;
	}
...
{
	if (!iRememberStatus) return GG_STATUS_NOT_AVAIL;
	return iAutoStatus;
}

void Profile::SetAutoStatus(int status)
{
	iAutoStatus = status;
}
