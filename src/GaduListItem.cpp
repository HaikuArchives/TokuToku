...
GaduListItem::GaduListItem(BString *aPerson, int aStatus, 
		BString *aDescription, BResources *aRes, Person *aPtr) 
	:
	BListItem(),
	iPerson(aPtr)
{
...
void 
GaduListItem::SetIcon(int aStatus)
{
	delete iIcon;
	iIcon = NULL;
...
void 
GaduListItem::DrawItem(BView* aOwner, BRect aFrame, bool aComplete)
{
	rgb_color color;
...
void 
GaduListItem::Update(BView* aOwner, const BFont* aFont)
{
	SetHeight(35.0);
}

BBitmap *
GaduListItem::GetBitmap(const char* aName)
{
	BBitmap *bitmap = NULL;
	size_t len = 0;
	status_t error;
